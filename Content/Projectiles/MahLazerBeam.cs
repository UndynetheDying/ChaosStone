...
{
	public class MahLazerBeam : ModProjectile
	{
		// A helpful math constant for performing beam angling calculations.
		private const float PiBeamDivisor = MathHelper.Pi / MAHLAZER.NumBeams;
		...
		public override void AI() {
			...
			if (Projectile.type != ModContent.ProjectileType<MahLazerBeam>() || !hostPrism.active || hostPrism.type != ModContent.ProjectileType<Projectiles.MAHLAZER>()) {
				Projectile.Kill();
				return;
			}
			...
			float chargeRatio = MathHelper.Clamp(hostPrism.ai[0] / MAHLAZER.MaxCharge, 0f, 1f);
			...
			Projectile.friendly = hostPrism.ai[0] > MAHLAZER.DamageStart;

			// This offset is used to make each individual beam orient differently based on its Beam ID.
			float beamIdOffset = BeamID - MAHLAZER.NumBeams / 2f + 0.5f;
			float beamSpread;
			float spinRate;
			...
			float deviationAngle = (hostPrism.ai[0] + beamIdOffset * spinRate) / (spinRate * MAHLAZER.NumBeams) * MathHelper.TwoPi;
		...
		private Color GetOuterBeamColor() {
			...
			float hue = (BeamID / MAHLAZER.NumBeams) % BeamHueVariance + BeamColorHue;
