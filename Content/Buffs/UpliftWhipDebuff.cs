...
{
	public class UpliftWhipDebuff : ModBuff
	{
	...
	public class UpliftWhipAdvancedDebuff : ModBuff
	{
	...
	public class UpliftWhipDebuffNPC : GlobalNPC
	{
		public override void ModifyHitByProjectile(NPC npc, Projectile projectile, ref NPC.HitModifiers modifiers) {
			...
			if (npc.HasBuff<UpliftWhipDebuff>()) {
				// Apply a flat bonus to every hit
				modifiers.FlatBonusDamage += OrderWhipDebuff.TagDamage * projTagMultiplier;
			}
			...
			if (npc.HasBuff<UpliftWhipAdvancedDebuff>()) {
				// Apply the scaling bonus to the next hit, and then remove the buff, like the vanilla firecracker
				modifiers.ScalingBonusDamage += OrderWhipAdvancedDebuff.TagDamageMultiplier * projTagMultiplier;
				npc.RequestBuffRemoval(ModContent.BuffType<UpliftWhipAdvancedDebuff>());
			}
		}
	}
}